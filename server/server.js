...
if (!isSqliteFile(DB_PATH)) {
  ...
  if (isSqliteFile(alt)) {
    ...
    if (fs.existsSync(path.join(__dirname, 'data.sqlite')) && !isSqliteFile(path.join(__dirname, 'data.sqlite'))) {
      try { fs.unlinkSync(path.join(__dirname, 'data.sqlite')); } catch (e) {}
    }
...
const TOKEN_MAX_AGE = parseInt(process.env.TOKEN_MAX_AGE || String(24 * 60 * 60 * 1000)); // ms
...
function requireAdmin(req, res, next) {
  ...
  if (!payload || payload.user !== ADMIN_USER) return res.status(401).json({ error: 'unauthorized' });
...
db.serialize(() => {
  ...
  db.get('SELECT COUNT(1) as cnt FROM state WHERE id = 1', (err, row) => {
    ...
    if (!row || row.cnt === 0) {
      db.run(`INSERT OR REPLACE INTO state (id, rows, cols, blocked, names) VALUES (1,?,?,?,?)`,
...
function getState(cb) {
  db.get('SELECT rows, cols, blocked, names FROM state WHERE id = 1', (err, row) => {
...
function saveState(state, cb) {
  db.run('UPDATE state SET rows = ?, cols = ?, blocked = ?, names = ? WHERE id = 1',
    [state.rows, state.cols, JSON.stringify(state.blocked || []), JSON.stringify(state.names || {})],
  ...
  for (let c = 0; c < cols; c++) {
    if (c % 2 === 0) {
      for (let r = rows - 1; r >= 0; r--) {
        const cid = `${r}-${c}`;
        if (!blockedSet.has(cid)) { coordToNumber[cid] = count; numberToCoord[count] = cid; count++; }
      }
    } else {
...
app.get('/state', (req, res) => {
  getState((err, state) => { if (err) return res.status(500).json({ error: err.message }); res.json(state); });
});

app.post('/generate', requireAdmin, (req, res) => {
  const { rows, cols } = req.body;
  if (!rows || !cols) return res.status(400).json({ error: 'rows and cols required' });
  const newState = { rows: parseInt(rows), cols: parseInt(cols), blocked: [], names: {} };
...
app.post('/update', requireAdmin, (req, res) => {
  ...
  getState((err, state) => {
    ...
    if (!targetCoord && visibility === 'hide') return res.status(400).json({ error: 'Box number not found or already hidden' });

    if (visibility === 'hide') {
      if (!state.blocked.includes(targetCoord)) state.blocked.push(targetCoord);
      delete state.names[boxNum];
    } else {
      if (!subtitle || subtitle.toString().trim() === '') delete state.names[boxNum]; else state.names[boxNum] = subtitle.toString().trim();
    }
...
app.post('/admin/login', (req, res) => {
  ...
  res.setHeader('Set-Cookie', `admin_token=${token}; HttpOnly; Path=/; Max-Age=${Math.floor(TOKEN_MAX_AGE/1000)}`);
  res.json({ ok: true });
});
...
io.on('connection', (socket) => {
  getState((err, state) => { if (!err) socket.emit('state', state); });
});
